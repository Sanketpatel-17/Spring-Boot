...
public class ProductDao {
	...
	public ProductBean getDataByProductId(int id) {
		...
		productBean=stmt.queryForObject("select * from product where productId= "+id, new BeanPropertyRowMapper<ProductBean>(ProductBean.class));
		return productBean;
	}
	...
	public boolean updateProduct(ProductBean productBean) {
		...
		int i=stmt.update("update product set name=? ,price=? where productId=?",productBean.getName(),productBean.getPrice(),productBean.getProductId());
		if(i==1)
			return true;
		else
			return false;
